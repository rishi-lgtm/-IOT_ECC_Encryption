...
r1 = requests.post("http://localhost:5000/t1", data={})
response=r1.text
print(response)
...
for count in range(1,10):
    if count == 1 :
        ...
        r = requests.post("http://localhost:5000/otp", data={'C1': c1,'C2':c2,'temp':float(temp),'count':count,'point':point,'public_key':public_key})
        print (r.text)
        ans='1'
        if ans == '1' :
            ...
            r1 = requests.post("http://localhost:5000/ecc", data={'qax': qax, 'qay': qay})
            print(r1.text)
        ...
        temp1 = (3 * temp1 + f + 3)/2
        temp = temp1 + k * public_key
        r = requests.post("http://localhost:5000/otp", data={'C1': c1,'C2':c2,'temp':float(temp+2),'count':count,'point':point,'public_key':public_key})
        ans=str(r.text)
        if ans == '1' :
            print("otp confirmed \nstarting connection\n ")
            gx = 126 # gx,gy are the coordinates of the generator point
